			...
			function displayPs(data) {
				// alert(data.wo.id);
				 ...
				 $("#model").text(data.wo.model.pt_desc1);
					...
					//$("#selectstatus").val(data.wo.status);
					...
					for (i = 0; i < data.lDetailComp.length; i++) {
					    ...
					    cell5.innerHTML = '<input  value="'+ data.lDetailComp[j].model.pt_part +'" name="lDetailComp['+ j +'].model.pt_part" type="hidden"><input class="qty" id="'+ data.lDetailComp[j].model.pt_part +'" class="inputtext" name="lDetailComp['+ j +'].qty" type="number" width="80" step="any">';
						   }else{
